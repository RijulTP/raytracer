...
int count = 0;
vec3 color(const ray& r,int xCoord,int yCoord,hitable *world,int depth) {
	hit_record rec;
	if (depth <= 0)
        return vec3(0,0,0);
	...
	if(world->hit(r,point,rec)){
		...
		return 0.5*color(ray(rec.p,target-rec.p),xCoord,yCoord,world,depth-1);
		//return 0.5*color(r,xCoord,yCoord,world);
...
int main() {
	...
	int triangle3[100][2] = { {0, 125}, {400, 125}, {600,125},{800,125},{800,0},{400,0},{0,0} };
	...
	for(int y=height-1; y>=00; y--) {
		for(int x=0; x<width; x++) {
			...
			vec3 col = color(r,x,y,world,max_depth);
